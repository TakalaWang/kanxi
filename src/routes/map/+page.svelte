<script lang="ts">
	...
	const groups = $derived.by(() => {
		const map: Record<string, { venue: Venue; shows: Show[] }> = {};
		let unlocated = 0;
		for (const s of data.shows) {
			...
			(map[v.name] ??= { venue: v, shows: [] }).shows.push(s);
		}
		return {
			list: Object.values(map).sort((a, b) => b.shows.length - a.shows.length),
			unlocated,
		};
	});
...
<header class="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3.5 sm:px-5">
	...
	<div class="flex items-center gap-2">
		<div
			class="flex overflow-hidden rounded-full border border-gray-200 text-sm dark:border-white/15"
		>
			<a
				href="/"
				class="px-3 py-1.5 text-gray-500 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-white/10"
				>清單</a
			>
			<a
				href="/calendar"
				class="border-l border-gray-200 px-3 py-1.5 text-gray-500 hover:bg-gray-100 dark:border-white/15 dark:text-gray-300 dark:hover:bg-white/10"
				>月曆</a
			>
			<span
				class="border-l border-gray-200 bg-curtain-600 px-3 py-1.5 font-medium text-white dark:border-white/15"
				>地圖</span
			>
		</div>
		<button
...
<main class="mx-auto grid max-w-6xl gap-4 px-4 pb-10 sm:px-5 lg:grid-cols-[1fr_22rem]">
	...
	<aside
		class="rounded-2xl border border-curtain-100 bg-white p-4 dark:border-white/10 dark:bg-[#1e1716]"
	>
		{#if activeVenue}
			<div class="mb-3 flex items-start justify-between gap-2">
				...
				<button
					onclick={() => (activeVenue = null)}
					aria-label="清除"
					class="text-gray-400 hover:text-curtain-600"
				>
					<Icon name="x" size={16} />
				</button>
			</div>
					...
					<li>
						...
						>
							...
							<span class="min-w-0 flex-1">
								<span class="line-clamp-2 font-medium text-gray-800 dark:text-gray-100"
									>{s.title}</span
								>
								<span class="mt-0.5 block text-xs text-gray-400"
									>{fmtDateRange(s)} · {SOURCE_LABELS[s.source]}</span
								>
							</span>
						</button>
					</li>
